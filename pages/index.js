...
const Index = () => {
  ...
  if(!data.obtenerClientesPorVendedor || error) return window.location.href = 'login';
          ...
          <tbody className="bg-white">
            {data.obtenerClientesPorVendedor.map(cliente=>(
              <tr key={cliente.id}>
                ...
                <td className='border px-4 py-2'>{cliente.telefono ? cliente.telefono : 'No se agrego numero'}</td>
              </tr>
            ))
          }
          </tbody>
